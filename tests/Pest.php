...
use Illuminate\Contracts\Support\Arrayable; // 需要 use
use Spatie\LaravelData\Casts\DateTimeInterfaceCast; // 需要 use
use Spatie\LaravelData\Casts\EnumCast; // 需要 use
use Spatie\LaravelData\Normalizers\ArrayableNormalizer; // 需要 use
use Spatie\LaravelData\Normalizers\ArrayNormalizer; // 需要 use
use Spatie\LaravelData\Normalizers\JsonNormalizer; // 需要 use
use Spatie\LaravelData\Normalizers\ModelNormalizer; // 需要 use
use Spatie\LaravelData\Normalizers\ObjectNormalizer; // 需要 use
use Spatie\LaravelData\RuleInferrers\AttributesRuleInferrer; // 需要 use
use Spatie\LaravelData\RuleInferrers\BuiltInTypesRuleInferrer; // 需要 use
use Spatie\LaravelData\RuleInferrers\NullableRuleInferrer; // 需要 use
use Spatie\LaravelData\RuleInferrers\RequiredRuleInferrer; // 需要 use
use Spatie\LaravelData\RuleInferrers\SometimesRuleInferrer; // 需要 use
use Spatie\LaravelData\Transformers\ArrayableTransformer; // 需要 use
use Spatie\LaravelData\Transformers\DateTimeInterfaceTransformer; // 需要 use
use Spatie\LaravelData\Transformers\EnumTransformer; // 需要 use

if (!function_exists('config')) {
    function config($key = null, $default = null)
    {
        if ('data' === $key || 'data.php' === $key) { // spatie/laravel-data might look for 'data' or 'data.php'
            return [
                /*
        ...
        // 对于其他可能的配置键，返回默认值或 null
        // 您可以根据需要扩展这个 shim 函数
        if (is_array($key)) { // 处理 config(['key' => 'value']) 这种设置方式 (尽管我们的场景主要是读取)
            // 在非 Laravel 环境中，这个 shim 主要用于读取，不太可能用于设置
            return;
        }
