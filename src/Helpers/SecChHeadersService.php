...
class SecChHeadersService
{
    ...
    public function generateSecChUa(): string
    {
        ...
        $isMobileChrome = 1 === preg_match('/Chrome.*Mobile.*Android/i', $this->userAgent);
        if ($isMobileChrome) {
        ...
        return match (true) {
            false !== stripos($browserName, 'Chrome') && false === stripos($browserName, 'Edge') => $this->generateChromeSecChUa($majorVersion),
            false !== stripos($browserName, 'Edge') || false !== stripos($browserName, 'Microsoft Edge') => $this->generateEdgeSecChUa($majorVersion),
            false !== stripos($browserName, 'Opera') => $this->generateOperaSecChUa($majorVersion, $this->extractChromeVersion()),
            false !== stripos($browserName, 'Chromium') => $this->generateGenericChromiumSecChUa($majorVersion),
            default => '"Not:A-Brand";v="24", "Chromium";v="133", "Google Chrome";v="133"'
        };
    }
    ...
    public function generateChromeSecChUa(string $majorVersion): string
    {
        ...
        return sprintf(
            '%s;v="%s", "Chromium";v="%s", "Google Chrome";v="%s"',
            $fakeBrand,
            $fakeVersion,
            $majorVersion,
            $majorVersion
    ...
    public function generateEdgeSecChUa(string $majorVersion): string
    {
        ...
        if ($majorVersionInt < 109) {
            return sprintf(
                '"Not A;Brand";v="99", "Chromium";v="%s", "Microsoft Edge";v="%s"',
                $majorVersion,
                $majorVersion
            );
        }
        ...
        return sprintf(
            '%s;v="%s", "Chromium";v="%s", "Microsoft Edge";v="%s"',
            $fakeBrand,
            $fakeVersion,
            $majorVersion,
            $majorVersion
    ...
    public function generateOperaSecChUa(string $operaVersion, string $chromeVersion): string
    {
        ...
        return sprintf(
            '"Opera";v="%s", "Not;A Brand";v="99", "Chromium";v="%s"',
            $operaVersion,
            $chromeVersion
        );
    }
